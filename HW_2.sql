...
-- Создаем условие отбора 'Тип заказа':
SELECT 
	id AS 'id заказа',
...
-- Проверяем наличие уже существующей таблицы с таким же названием, если имеется - удаляем и создаем заново с указанными полями:
...
-- NULL - это отсутствие значений в ячейке, а 0 это цифра, т.е. значение (с которым можно производить математические вычисления).